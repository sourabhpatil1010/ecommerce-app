...
export function ProductCard({ product, categoryName, onEdit, onDelete }: ProductCardProps) {
  ...
  return (
    <div className="group flex flex-col relative transition-transform duration-300 hover:-translate-y-1" data-testid="product-card">
      
      {/* Image Container */}
      <Link to={`/products/${product.id}`} data-testid="product-image-link" className="relative block w-full aspect-square overflow-hidden bg-gray-100 dark:bg-gray-900 rounded-2xl mb-4 group-hover:shadow-2xl transition-all duration-300">
        <img
          src={displayImage}
          alt={product.name}
      ...
      <div className="flex flex-col gap-1 px-1">
        <Link to={`/products/${product.id}`} data-testid="product-title-link" className="flex justify-between items-start gap-4">
